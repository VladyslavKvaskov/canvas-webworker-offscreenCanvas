...
function eat(d) {
    if (d) {
        ...
        async function moveBug() {
            function drawBug() {
                let arr = data.filter((g) => g.y + g.containerHeight > bug.y && g.x + g.containerWidth > bug.x && g.y < bug.y + g.containerHeight && g.x < bug.x + g.containerWidth);
            ...
            if (bug[moveXY === 'x' ? 'x' : 'y'] + bug.increament > d[moveXY === 'x' ? 'containerX' : 'containerY'] && bug[moveXY === 'x' ? 'x' : 'y'] - bug.increament < d[moveXY === 'x' ? 'containerX' : 'containerY']) {
                bug[moveXY === 'x' ? 'x' : 'y'] = d[moveXY === 'x' ? 'containerX' : 'containerY'];
                drawBug();
                if (bug[moveXY === 'x' ? 'y' : 'x'] + bug.increament > d[moveXY === 'x' ? 'containerY' : 'containerX'] && bug[moveXY === 'x' ? 'y' : 'x'] - bug.increament < d[moveXY === 'x' ? 'containerY' : 'containerX']) {
                    bug[moveXY === 'x' ? 'y' : 'x'] = d[moveXY === 'x' ? 'containerY' : 'containerX'];
                    drawBug();
                    ...
                    if (bug[moveXY === 'x' ? 'y' : 'x'] < d[moveXY === 'x' ? 'containerY' : 'containerX']) {
                        bug[moveXY === 'x' ? 'y' : 'x'] += bug.increament;
                    } else if (bug[moveXY === 'x' ? 'y' : 'x'] > d[moveXY === 'x' ? 'containerY' : 'containerX']) {
                        bug[moveXY === 'x' ? 'y' : 'x'] -= bug.increament;
                    }
                ...
                if (bug[moveXY === 'x' ? 'x' : 'y'] < d[moveXY === 'x' ? 'containerX' : 'containerY']) {
                    bug[moveXY === 'x' ? 'x' : 'y'] += bug.increament;
                } else if (bug[moveXY === 'x' ? 'x' : 'y'] > d[moveXY === 'x' ? 'containerX' : 'containerY']) {
                    bug[moveXY === 'x' ? 'x' : 'y'] -= bug.increament;
                }
...
function eatRandom(par = false) {
    ...
    if (arrToEat.length > 0) {
        ...
        // console.log(arrToEat[getRndInteger(0, arrToEat.length - 1)]);
